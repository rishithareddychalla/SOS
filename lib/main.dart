...
//       final prefs = await SharedPreferences.getInstance();
//       final contactsJson = prefs.getString('contacts');
//       debugPrint('Loading contacts from SharedPreferences: $contactsJson');
//       if (contactsJson != null) {
//         final decoded = json.decode(contactsJson) as List;
//         final contacts = decoded.map((item) => Map<String, String>.from(item)).toList();
//         state = AsyncValue.data(contacts);
//         debugPrint('Successfully loaded contacts: $contacts');
//       } else {
//         state = const AsyncValue.data([]); // Set empty list if no data
//         debugPrint('No contacts found in SharedPreferences');
//       }
...
//       state = AsyncValue.data(newContacts.cast<Map<String, String>>());
//       await _saveContacts();
//       debugPrint('Contact added, new state: $newContacts');
//     } catch (e) {
//       state = AsyncValue.error(e, StackTrace.current);
//       debugPrint('Error adding contact: $e');
//       debugPrint('Stack trace: ${StackTrace.current}');
...
//         state = AsyncValue.data(newContacts.cast<Map<String, String>>());
//         await _saveContacts();
//         debugPrint('Contact deleted, new state: $newContacts');
//       }
//     } catch (e) {
//       state = AsyncValue.error(e, StackTrace.current);
//       debugPrint('Error deleting contact: $e');
...
//       await prefs.setString('contacts', json.encode(contactsToSave));
//       final savedJson = prefs.getString('contacts');
//       debugPrint('Contacts saved to SharedPreferences: $savedJson');
//       if (savedJson != json.encode(contactsToSave)) {
//         debugPrint('Warning: Saved data does not match current state');
//       }
//     } catch (e) {
...
//     // Iterate through contacts with a timeout for each call
//     for (int i = 0; i < contactList.length; i++) {
//       final contact = contactList[i];
//       bool? callResult = await FlutterPhoneDirectCaller.callNumber(contact['phone']!);

//       if (callResult != true) {
//         debugPrint('Failed to initiate call to ${contact['phone']}');
//         ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(content: Text('Failed to initiate call.')),
//         );
//         continue; // Move to next contact if call initiation fails
//       }

//       // Simulate waiting for the call to be answered (20-second timeout)
//       await Future.delayed(const Duration(seconds: 20), () {
//         debugPrint('Timeout reached for ${contact['phone']}. Moving to next contact.');
//       }).then((_) {
//         // After timeout, check if we should move to the next contact
//         if (i < contactList.length - 1) {
//           debugPrint('Calling next contact: ${contactList[i + 1]['phone']}');
//           return; // Let the loop continue to the next iteration
//         } else {
//           // No more contacts, show a message
//           ScaffoldMessenger.of(context).showSnackBar(
//             const SnackBar(content: Text('No more contacts to call.')),
//           );
//         }
//       });

//       // If we reach here and the loop continues, the call timed out, and the next iteration will start
//     }

//     // If at least one call was initiated, navigate to CallingScreen after the last attempt
//     if (contactList.isNotEmpty) {
//       await Future.delayed(const Duration(seconds: 3)); // Brief delay after last call
//       context.go('/calling', extra: contactList.last); // Use the last called contact
//     }
//   }
...
//                       const SizedBox(height: 20),
//                       const Text(
//                         'Tapping SOS calls your emergency contacts sequentially (20s timeout)',
//                         textAlign: TextAlign.center,
//                         style: TextStyle(fontSize: 16, color: Colors.black54),
//                       ),
//                     ],
